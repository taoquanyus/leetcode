...
public class Solution {
    //在这里解释一下：
    //首先先进行数据预处理，把关系数组转化为出入度数组和表示网络关系的二维列表
    //然后将出入度为1的节点加入队列遍历
    //每次遍历完都会剪枝，出入度为2的变成出入度为1的队列
    //相当于把原来的图给剪掉一圈叶子节点，形成一个缩小的新的图吗
    //最后剩下的，就是原先出入度最高的变量
    public class FindMinHeightTrees {//超出了我暂时的能力范围，但是算法原理是看懂了
        public List<Integer> findMinHeightTrees(int n, int[][] edges) {
            ...
            这样最后保存的就是最终的最小高度树了*/
                ...
                不就相当于把原来的图给剪掉一圈叶子节点，形成一个缩小的新的图吗*/
                    List<Integer> neighbors = map.get(cur);//map.get(cur)表示的是当前节点连接的节点们
